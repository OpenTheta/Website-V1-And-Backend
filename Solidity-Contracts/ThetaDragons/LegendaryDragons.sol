...
contract LegendaryDragons is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {

    address ThetaDragons = 0xc6A368bCDc89E0A9B776efc70f841995455C5a69;

    string private baseURI;

    uint256 public MAX_NFT_SUPPLY = 1111;
    ...
    uint public promoEndTimestamp;
    bool public promoIsActive = false;

    // randomize mint
    uint16[1111] private order;
    uint private start;
    uint private end;
    ...
    constructor(address _fee, address _creator, string memory _uri) ERC721("Legendary Dragons", "LD") {
        feeAddress = _fee;
...
* @dev Gets current Price
    */
    function getDiscountedNFTPrice(address buyer) public view returns (uint256) {
        uint multiplier = 100;
        if (!promoIsActive || block.timestamp > promoEndTimestamp) {
            multiplier = 100;
        }
        else if(IERC721(ThetaDragons).balanceOf(buyer) >= 15) {
            multiplier -= 15;
        } else if(IERC721(ThetaDragons).balanceOf(buyer) >= 5) {
            multiplier -= 10;
        }
    ...
    */
    function safeMint(address to) public payable {
        ...
        uint256 creatorPayout = (msg.value / 100) * 75;
    ...
    */
    function setPromoState(bool _isActive, uint _endTimestamp) public onlyOwner {
        promoIsActive = _isActive;
        promoEndTimestamp = _endTimestamp;
    }
