...
contract Panamals is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {

    address public discountNFT = 0x606BD92F6914E27D8aE4F3299ddD796392c68178;
    uint public discountNFTAmount = 5;

    string private baseURI;

    uint256 public MAX_NFT_SUPPLY = 325;
    ...
    uint256 private price = 240000000000000000000;
    bool public promoIsActive = false;

    // randomize mint
    uint16[325] private order;
    uint private start;
    uint private end;
    ...
    constructor(address _fee, address _creator, string memory _uri) ERC721("Panamals", "PANAMA") {
        feeAddress = _fee;
...
* @dev Gets current Price
    */
    function getDiscountedNFTPrice(address buyer) public view returns (uint256) {
        ...
        else if(IERC721(discountNFT).balanceOf(buyer) >= discountNFTAmount) {
            multiplier -= 5;
        }
    ...
    */
    function reserveToAddresses(address[] memory _senderAddress) public onlyOwner {
        require(orderIsShuffled, "Not shuffled");
        require(totalSupply()+_senderAddress.length <= MAX_NFT_SUPPLY, "Not enough tokens available");
...
*/
    function safeMintQuantity(address to, uint8 quantity) public payable {
        require(saleIsActive, "Sale must be active to mint");
        require(totalSupply() + quantity <= MAX_NFT_SUPPLY, "Purchase would exceed max supply");
        if(promoIsActive) {
            require(getDiscountedNFTPrice(to)  * quantity <= msg.value, "Wrong value");
        } else {
    ...
    */
    function setPromoState(bool _isActive) public onlyOwner {
        promoIsActive = _isActive;
    }
    ...
    */
     ...
     * Change discount NFT
    */
    function changeDiscount(address _discountNFT, uint _amount) onlyOwner public {
        discountNFT = _discountNFT;
        discountNFTAmount = _amount;
    }
