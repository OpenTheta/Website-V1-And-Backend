...
contract ThetaPEARLS is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {

    bool public mintingIsActive = false;
    uint public MAX_NFT_SUPPLY = 900;

    address public feeAddress;
    address public creatorAddress;
    uint public PRICE = 110000000000000000000;

    //    mapping Address to access to mint
    mapping(address => bool) private AddressIsMinter;

    constructor(address _fee, address _creator) ERC721("Theta PEARLS", "T-PEARLS") {
        feeAddress = _fee;
        creatorAddress = _creator;
    }

    modifier onlyMinter {
        require(AddressIsMinter[msg.sender], "Only Minters can mint new tokens");
        _;
    }
    ...
    */
    function safeMint(address to, string memory uri) external onlyMinter {
        ...
        require(totalSupply() < MAX_NFT_SUPPLY, "tokenId not in range");
    ...
    * Set some NFTs aside
    */
    function reserveNFTS(uint256 numberOfNfts, address _senderAddress, uint startJsonId, string memory baseURI) public onlyOwner {
        require(totalSupply()+numberOfNfts <= MAX_NFT_SUPPLY, "To many NFTs would exist");
        for (uint i = 0; i < numberOfNfts; i++) {
            ...
            if (supply < MAX_NFT_SUPPLY )
            {
                ...
                string memory id = Strings.toString(startJsonId+i);
    ...
    function setMinter(address minter, bool access) public onlyOwner {
        AddressIsMinter[minter] = access;
    }

    function isMinter(address minterAddress) public view returns(bool) {
        return AddressIsMinter[minterAddress];
    }
    ...
    */
    ...
    function updateURI(uint tokenId, string memory uri) onlyOwner public {
