...
contract MATRESHKA is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {

    uint256 public MAX_NFT_SUPPLY = 1000;
    string private baseURI;
    uint16[888] private order;
    uint private start;
    uint private end;
    uint private stepSize;
    bool private orderIsCreated;
    bool private orderIsShuffled;
    uint lastToMintToken = 888;
    uint reservedToken;
    uint mintedAmount;
    uint price = 888000000000000000000;
    address public ThetaTeeth = 0x23a185f6cf673D74F3DD69086f20136Ee30e7129;

    bool public saleIsActive = false;
    bool public presale = true;
    address public feeAddress;

    constructor(address fee, string memory _uri) ERC721("MATRESHKA", "M") {
        baseURI = _uri;
        feeAddress = fee;
        uint _stepSize = lastToMintToken; //888
        stepSize = _stepSize;
        end = stepSize;
    }
    ...
    function shuffle(uint seed) external {
        ...
        if(end > lastToMintToken) end = lastToMintToken;
    ...
    */
    function getDiscountedNFTPrice(address buyer) public view returns (uint256) {
        ...
        if(presale && ERC721(ThetaTeeth).balanceOf(buyer) > 31) {
            return price*70/100;
        } else if(presale && ERC721(ThetaTeeth).balanceOf(buyer) > 15) {
            return price*80/100;
        } else if(presale && ERC721(ThetaTeeth).balanceOf(buyer) > 0) {
            return price*90/100;
        }  else {
            require(!presale, "Sale has not started");
            return price;
        }
    }

    function hasDiscount(address buyer) public view returns(bool) {
        return (ERC721(ThetaTeeth).balanceOf(buyer) > 0 && presale);
    }
    ...
    */
    function reserveNFTS(uint256 numberOfNfts, address _receiverAddress) public onlyOwner {
        ...
        for (uint i = 0; i < numberOfNfts; i++) {
            ...
            if (lastToMintToken + reservedToken < MAX_NFT_SUPPLY )
            {
                reservedToken += 1;
                uint256 tokenId = lastToMintToken + reservedToken;
    ...
    */
    function safeMint(address to) public payable {
        ...
        require(mintedAmount < lastToMintToken, "Purchase would exceed max supply");
        require(hasDiscount(to) || !presale);
        ...
        uint256 tokenId = order[mintedAmount];
        mintedAmount++;
