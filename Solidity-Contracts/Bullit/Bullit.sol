...
contract Bullit is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    mapping(address => bool) addressHasDiscount;
    ...
    * @dev Gets current Price
    ...
    function hasDiscount(address buyer) public view returns(bool) {
        return addressHasDiscount[buyer];
    }
    ...
    */
    function getDiscountedNFTPrice(address buyer) public view returns (uint256) {
        if(addressHasDiscount[buyer]) {
            require(totalSupply() < MAX_NFT_SUPPLY, "Sale has already ended");
            return 0; // 144
        } else {
            require(totalSupply() < MAX_NFT_SUPPLY-numberOfDiscounts, "Purchase would exceed max supply");
            return 600000000000000000000; // 1 - 856 600 TFUEL
        }
    }
    ...
    * Set some NFTs aside
    */
    function setDiscountAddresses(address[] memory _discountAddresses) public onlyOwner {
        for (uint i = 0; i < _discountAddresses.length; i++) {
            uint256 supply = totalSupply();

            if (supply+numberOfDiscounts < MAX_NFT_SUPPLY) {
                if(!addressHasDiscount[_discountAddresses[i]]) {
                    numberOfDiscounts++;
                    addressHasDiscount[_discountAddresses[i]] = true;
                }
            }
        }
    }
    ...
    */
    function safeMint(address to) public payable {
        ...
        uint256 price = getDiscountedNFTPrice(to);
        if(price == 0) {
            addressHasDiscount[to] = false;
            numberOfDiscounts -= 1;
        } else {
            require(totalSupply() < MAX_NFT_SUPPLY-numberOfDiscounts, "Purchase would exceed max supply");
