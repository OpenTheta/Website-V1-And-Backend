...
contract MeepHeadz is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, Ownable {
    ...
    constructor(address fee, string memory uri) ERC721("MeepHeadz", "MH") {
        feeAddress = fee;
        baseURI = uri;
    }
    ...
    */
    function setAdmin(address admin_) onlyOwner external {
        admin = admin_;
    }

    /**
    * @notice Gets current Price
    */
    function getNFTPrice() public view returns (uint256) {
        uint currentSupply = totalSupply();
        require(currentSupply < MAX_NFT_SUPPLY, "Sale has already ended");

        return 100000000000000000000; // 1 - 10000 100 TFUEL
    }

    /**
    * @notice Set some NFTs aside
    */
    function reserveNFTS(uint256 numberOfNfts, address to) public onlyAdmin {
        require(_tokenIdCounter.current() + numberOfNfts <= MAX_NFT_SUPPLY, "Would exceed max supply");
    ...
    /**
    * @notice send specific addresses free NFTs
    */
    function reserveToAddresses(address[] memory _senderAddress) public onlyAdmin {
        require(_tokenIdCounter.current() + _senderAddress.length <= MAX_NFT_SUPPLY, "Would exceed max supply");
    ...
    /**
    * @notice public function to mint NFT for TFuel
    */
    function safeMint(address to) public payable{
        require(saleIsActive, "Sale must be active to mint");
        require(_tokenIdCounter.current() < MAX_NFT_SUPPLY, "Purchase would exceed max supply");
    ...
    */
    function safeMintQuantity(address to, uint8 quantity) public payable {
        require(saleIsActive, "Sale must be active to mint");
        require(_tokenIdCounter.current() + quantity <= MAX_NFT_SUPPLY, "Purchase would exceed max supply");
        require(getNFTPrice() * quantity == msg.value, "TFuel value sent is not correct");
