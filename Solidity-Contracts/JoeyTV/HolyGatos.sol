...
contract HolyGatos is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {

    constructor() ERC721("Holy Gato's", "HG") {}

    /**
    * send specific addresses free NFTs
    */
    function reserveToAddresses(uint startId, uint256 numberOfNfts, string memory uri, address[] memory _senderAddress) public onlyOwner {
        uint tokenId = startId;
        for (uint i = 0; i < numberOfNfts; i++) {
            tokenId = tokenId + 1;
            safeMint(_senderAddress[i], uri, tokenId);
        }
    }
    ...
    */
    function safeMint(address to, string memory uri, uint256 tokenId) internal {
        _safeMint(to, tokenId);
