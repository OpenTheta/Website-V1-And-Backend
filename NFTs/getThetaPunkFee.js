...
let total = 0;
let StartTime = new Date('January 1, 2022 00:00:00').getTime();
let EndTime = new Date('February 1, 2023 00:00:00').getTime();

axios.get(url).then(response => {
    for(let i=0; i< response.data.length; i++) {
        // console.log(response.data[i].soldTimestamp - time)
        if(EndTime > response.data[i].soldTimestamp && response.data[i].soldTimestamp > StartTime && response.data[i].creator === 'ThetaCon'){
